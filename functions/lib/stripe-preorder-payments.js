...
exports.cancelPreorderPayment = exports.getPreorderPaymentStatus = exports.createPreorderPaymentIntent = void 0;
 ...
 */
exports.createPreorderPaymentIntent = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    try {
        ...
        if (!stripeAccountId) {
            throw new functions.https.HttpsError('failed-precondition', 'Vendor has not connected Stripe. Please ask vendor to connect their Stripe account in settings.');
        }
        // Check if vendor is fully verified
        const chargesEnabled = integrationDoc.data()?.stripe?.chargesEnabled;
        if (!chargesEnabled) {
            throw new functions.https.HttpsError('failed-precondition', 'Vendor Stripe account is not fully verified yet. Please try again later.');
        }
        ...
        const orderRef = await admin.firestore().collection('preorders').add({
            ...
            totalAmount: shopperTotalCents / 100,
 ...
 */
exports.getPreorderPaymentStatus = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const { orderId } = data;
 ...
 */
exports.cancelPreorderPayment = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const { orderId } = data;
    try {
        ...
        if (orderData.transferId) {
            throw new functions.https.HttpsError('failed-precondition', 'Cannot cancel - funds already transferred to vendor');
        }
        // Check if payment succeeded
        if (orderData.status !== 'paid' && orderData.status !== 'pending_payment') {
            throw new functions.https.HttpsError('failed-precondition', `Cannot cancel order with status: ${orderData.status}`);
        }
...
//# sourceMappingURL=stripe-preorder-payments.js.map