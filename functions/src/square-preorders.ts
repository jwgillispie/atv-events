 ...
 */
export const createSquarePreorderCheckout = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
    ...
    const squareData = squareDoc.data() as SquareIntegration;
 ...
 */
export const handleSquareOrderWebhook = functions.https.onRequest(
  async (req, res) => {
      ...
      if (!preorderQuery.empty) {
        ...
        if (status === 'paid') {
          ...
          await db.collection('vendorSales').add({
            ...
            orderId: order.id,
            provider: 'square',
            totalAmount: order.total_money.amount / 100, // Convert cents to dollars
 ...
 */
export const fetchSquareCatalog = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    ...
    const products = catalogData.objects
      ?.filter((obj: any) => obj.type === 'ITEM' && obj.item_data.is_archived === false)
      .map((item: any) => ({
        id: item.id,
