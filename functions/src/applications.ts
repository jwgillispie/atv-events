 ...
 */
export const expireApplicationsScheduled = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    try {
      ...
      const expiredApplications: Array<{
        id: string;
        vendorId: string;
        marketId: string;
        marketName: string;
...
async function handleApplicationApproved(data: {
  ...
  vendorName: string;
  marketName: string;
  approvalExpiresAt: admin.firestore.Timestamp;
  totalFee: number;
}) {
...
async function sendExpirationNotification(data: {
  ...
  await handleApplicationExpired({
    ...
    vendorId: data.vendorId,
    organizerId: '', // Will be fetched in handler
