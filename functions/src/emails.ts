...
// Email templates
const getOrderConfirmationTemplate = (order: any, qrCodeUrl: string) => {
...
const getVendorOrderNotificationTemplate = (order: any) => {
  const pickupDate = new Date(order.pickupDate._seconds * 1000);
  const formattedDate = pickupDate.toLocaleDateString('en-US', {
    weekday: 'long',
    year: 'numeric',
...
const getTicketConfirmationTemplate = (ticket: any, qrCodeUrl: string) => {
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      const vendorMailOptions = {
        from: getFromEmail(),
        to: order.vendorEmail || SUPPORT_EMAIL, // Fallback to support if vendor email not available
...
export const sendTicketConfirmationEmail = functions.firestore
...
export const sendOrderStatusUpdateEmail = functions.firestore
  .document('orders/{orderId}')
...
export const testEmailFunction = functions.https.onRequest(async (req, res) => {
  if (req.method !== 'POST') {
...
const getBrandedEmailTemplate = (messageBody: string, userType: string, userName: string) => {
...
export const sendEmailBlast = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    for (let i = 0; i < recipients.length; i += BATCH_SIZE) {
      ...
      const sendPromises = batch.map(async (recipient: any) => {
        try {
...
const getVendorApplicationConfirmationTemplate = (application: any, marketName: string) => {
  return `
    <!DOCTYPE html>
    <html>
    <head>
...
export const sendVendorApplicationConfirmationEmail = functions.firestore
